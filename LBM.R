LBM <- function(WI, SI, 
  ...
  ...) {
    ...
    Cultivar <- toupper(SI$Sfile$Cultivar[1])
    ApplSys <- toupper(SI$Sfile$ApplSys[1])
    ...
    Wfile<-WI$Wfile
    Sfile<-SI$Sfile
    EmergDate<-WI$EmergDate
    EndEpidDate<-WI$EndEpidDate
    # Start model lateblight
    SeLenght <- nrow(Wfile)
    # Obtaining nInocDay
    nInocDayFile <-subset(Wfile, Wfile$Date <= InocDate)
    nInocDay     <- nrow( nInocDayFile )
    # End obtaining nInocDay
    # Obtaining date after emergence
    nday<-as.numeric(Wfile$Date-EmergDate)
    ModelFile<-data.frame(EmergDate,Date=Wfile$Date,nday,InocDate,EndEpidDate)
    ...
    for( nday in 2:(SeLenght)) {
      ...
      if( !is.na( LfWetTmp ) ){
        if ((LfWetTmp>=14.0) & (LfWetTmp <= 29.0)) {
          TDirect <- 2.1 +LfWetTmp *(-.48 + LfWetTmp*(.031 + LfWetTmp*(-4.5E-04 -4.4E-06*LfWetTmp)))
        ...
        if ((LfWetTmp >= 0.0) & (LfWetTmp <= 24.0)) {
          TIndir <- 1.314E-02 + LfWetTmp * (6.98E-02 + LfWetTmp*(1.815E-02 + LfWetTmp * (-2.23E-03 + LfWetTmp * (7.48E-05 - 7.68E-07 * LfWetTmp))))
        ...
        if (( LfWetTmp >= 0.0) & ( LfWetTmp <= 28)) {
          TZooGrm <- 4.94E-03 + LfWetTmp*(.364 + LfWetTmp*(-5.813E-02 + LfWetTmp*(4.52E-03 + LfWetTmp*(-1.617E-04 + 2.068E-06 * LfWetTmp))))
      ...
      if( !is.na( HumidTmp[nday] ) ){
        ...
        if (( HumidTmp[nday] >= 1.5) & (HumidTmp[nday] <= 30.0) ) {
          fTRH <- -.4846 + HumidTmp[nday] * (.3735 + HumidTmp[nday]*(-3.556E-02 + HumidTmp[nday] * (1.509E-03 - 2.405E-05 * HumidTmp[nday])))
      ...
      LeafFct <- ifelse(BlPr[nday] < 0.82, exp (-2.1 * BlPr[nday-1]), exp (-2.1 * 0.9 * BlPr[nday-1]))
      ...
      n5DayPer <- nday %/% 5 + 1
      Divisor <- n5DayPer - 1
      if (MatTime=="EARLYSEASON")LAI[nday]<-Early[n5DayPer]+(((Early[n5DayPer + 1] - Early[n5DayPer]) / 5) * (nday-5*Divisor))
      if ((MatTime == "EARLYSEASON")&(nday > 44))LAI[nday] <- 2.5
      if (MatTime=="MIDSEASON")LAI[nday]<- Mid[n5DayPer] + (((Mid[n5DayPer + 1] - Mid[n5DayPer]) / 5) * (nday - 5 * Divisor))
      if ((MatTime == "MIDSEASON") &(nday > 52)) LAI[nday] <- 3.5
      if (MatTime=="LATESEASON")LAI[nday]<-Late[n5DayPer]+ (((Late[n5DayPer + 1] - Late[n5DayPer]) / 5) * (nday - 5 * Divisor))
...
#    Gfile <-subset(Ofile,((Ofile$ApplSys == ApplSys )& ( Ofile$Cultivar == Cultivar)))
    ...
    Gfile <- data.frame(Sfile, MinObs,MaxObs)
    plot(Ofile$nday,Ofile$SimSeverity,
         xlab="days after planting",ylab="Severity (%)")
    points(Gfile$nday,Gfile$MeanSeverity,col="blue",cex=1,pch=19)
    npoints <- nrow( Gfile )
    for ( i in 1:npoints){
      segments(Gfile$nday[i],Gfile$MinObs[i],Gfile$nday[i],Gfile$MaxObs[i],lwd=1.5,col="blue")
    }
